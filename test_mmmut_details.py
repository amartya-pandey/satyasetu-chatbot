import asyncio
from motor.motor_asyncio import AsyncIOMotorClient
from bson import ObjectId

async def test_mmmut_details():
    """Test with MMMUT which has more data"""
    import os
    from dotenv import load_dotenv
    load_dotenv()
    
    mongodb_url = os.getenv('MONGODB_URL')
    client = AsyncIOMotorClient(mongodb_url)
    db = client["SatyaSetu"]
    
    # Find MMMUT institution
    institution = await db.institutions.find_one({"name": {"$regex": "MMMUT", "$options": "i"}})
    
    if not institution:
        print("MMMUT institution not found!")
        # Try finding by ID from previous certificates
        cert = await db.certificates.find_one()
        if cert:
            institution_id = cert.get('institutionId')
            institution = await db.institutions.find_one({"_id": institution_id})
    
    if not institution:
        print("No institution found!")
        return
    
    org_id = institution.get('_id')
    
    print("=" * 80)
    print(f"INSTITUTION: {institution.get('name')}")
    print(f"ID: {org_id}")
    print("=" * 80)
    
    # Get all users linked to this institution
    all_users = await db.users.find({"organization": org_id}).to_list(length=1000)
    
    sub_admins = [u for u in all_users if u.get('role') in ['ADMIN', 'SUBADMIN', 'INSTITUTION_ADMIN', 'INSTITUTION']]
    regular_users = [u for u in all_users if u.get('role') not in ['ADMIN', 'SUBADMIN', 'INSTITUTION_ADMIN', 'INSTITUTION']]
    
    print(f"\nTotal Users: {len(all_users)}")
    print(f"Sub-Admins: {len(sub_admins)}")
    print(f"Regular Users: {len(regular_users)}")
    
    if sub_admins:
        print("\n" + "=" * 80)
        print("SUB-ADMINS:")
        print("=" * 80)
        for admin in sub_admins:
            print(f"  • {admin.get('name')} ({admin.get('email')})")
            print(f"    Role: {admin.get('role')}")
            print(f"    ID: {admin.get('_id')}")
            if admin.get('meta'):
                print(f"    Meta: {admin.get('meta')}")
            print()
    
    # Get certificates
    total_certs = await db.certificates.count_documents({"institutionId": org_id})
    print("\n" + "=" * 80)
    print(f"CERTIFICATES ISSUED: {total_certs}")
    print("=" * 80)
    
    if total_certs > 0:
        certs = await db.certificates.find({"institutionId": org_id}).limit(5).to_list(length=5)
        print("\nSample Certificates:")
        for i, cert in enumerate(certs, 1):
            print(f"\n{i}. {cert.get('certificateId')}")
            print(f"   Student: {cert.get('student', {}).get('fullName')}")
            print(f"   Course: {cert.get('student', {}).get('course')} - {cert.get('student', {}).get('department')}")
            
            # Get issuer details
            issuer_id = cert.get('issuerId')
            if issuer_id:
                issuer = await db.users.find_one({"_id": issuer_id})
                if issuer:
                    print(f"   Issued By: {issuer.get('name')} ({issuer.get('email')}) - {issuer.get('role')}")
    
    client.close()

asyncio.run(test_mmmut_details())
